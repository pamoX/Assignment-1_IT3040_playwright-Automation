...
```bash
npx playwright test tests/all_fun.spec.js
```
...
Note: `all_fun.spec.js` uses CommonJS (`require`) while `example.spec.js` uses `import` (ESM). See the Troubleshooting section below if you encounter module-related errors.
...
- Tests failing due to selectors or site changes
  - The tests rely on the live site `https://www.swifttranslator.com/`. If the site changes selectors/structure, update the selectors in `tests/all_fun.spec.js`.
...
1. Add new entries to `positiveTests` or `negativeTests` arrays in `tests/all_fun.spec.js` following the existing pattern, or
